...
export default function CheckoutRedirectConfig({ funnelId, pageId, onUpdate }: CheckoutRedirectConfigProps) {
    const { t } = useI18n()
    ...
    const fetchData = async () => {
        try {
            ...
            if (allPages) {
                // Filter out current page and show only pages after checkout
        ...
        <div className="bg-white dark:bg-[#0f1117] rounded-lg border border-gray-200 dark:border-zinc-800 p-4">
            ...
            <div className="space-y-3">
                <div>
                    ...
                    <select
                        value={loginUrl && settings.post_purchase_redirect_url === loginUrl ? '_login' : (settings.post_purchase_page_id || (settings.post_purchase_redirect_url ? '_custom' : '_thankyou'))}
                        onChange={(e) => {
                            const value = e.target.value
                            if (value === '_custom') {
                                setSettings(prev => ({ ...prev, post_purchase_page_id: undefined, post_purchase_redirect_url: '' }))
                            } else if (value === '_thankyou') {
                                setSettings(prev => ({ ...prev, post_purchase_page_id: undefined, post_purchase_redirect_url: undefined }))
                            } else if (value === '_login') {
                                setSettings(prev => ({ ...prev, post_purchase_page_id: undefined, post_purchase_redirect_url: loginUrl || '' }))
                            } else {
                                setSettings(prev => ({ ...prev, post_purchase_page_id: value, post_purchase_redirect_url: undefined }))
                            }
                        }}
                    ...
                    >
                        <option value="_thankyou">{t('funnel_components.checkout_redirect.thankyou_default')}</option>
                        {pages.filter(p => p.page_type !== 'thankyou').map(p => (
                    ...
                    {!settings.post_purchase_page_id && settings.post_purchase_redirect_url !== undefined && !(loginUrl && settings.post_purchase_redirect_url === loginUrl) && (
                        <div className="mt-1.5 relative">
                            ...
                            <input
                                ...
                                onChange={(e) => setSettings(prev => ({ ...prev, post_purchase_redirect_url: e.target.value || undefined }))}
                    ...
                    <p className="text-[10px] text-zinc-500 mt-2">
                        {t('funnel_components.checkout_redirect.skip_thankyou_tip')}
                    </p>
                </div>
