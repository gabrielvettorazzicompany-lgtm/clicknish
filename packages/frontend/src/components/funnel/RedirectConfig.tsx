...
export default function RedirectConfig({ funnelId, pageId, pageType, onUpdate, onRedirectsChanged }: RedirectConfigProps) {
    const { t } = useI18n()
    ...
    const fetchData = async () => {
        try {
            ...
            if (allPages) {
                // Filter out current page and pages before it
                const currentPosition = currentPage?.position ?? 0
                const availablePages = allPages.filter(p => p.id !== pageId && p.position > currentPosition)
                ...
                <div>
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            if (value === '_custom') {
                                setSettings(prev => ({ ...prev, accept_page_id: undefined, accept_redirect_url: '' }))
                            } else if (value === '_login') {
                                setSettings(prev => ({ ...prev, accept_page_id: undefined, accept_redirect_url: loginUrl || '' }))
                            } else if (value === '_none') {
                                setSettings(prev => ({ ...prev, accept_page_id: undefined, accept_redirect_url: undefined }))
                            } else {
                                setSettings(prev => ({ ...prev, accept_page_id: value, accept_redirect_url: undefined }))
                            }
                        }}
                        className="w-full px-3 py-2 bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-800 rounded text-gray-900 dark:text-white text-xs focus:outline-none focus:border-blue-500 dark:focus:border-zinc-600 appearance-none cursor-pointer transition-colors"
                    >
                        <option value="_none">{t('funnel_components.auto_next_page')}</option>
                        {pages.map(p => (
                            <option key={p.id} value={p.id}>
                                {getPageIcon(p.page_type)} {p.name} ({getPageLabel(p.page_type)})
                            </option>
                        ))}
                    ...
                    {!settings.accept_page_id && settings.accept_redirect_url !== undefined && settings.accept_redirect_url !== null && !(loginUrl && settings.accept_redirect_url === loginUrl) && (
                        <div className="mt-1.5 relative">
                            ...
                            <input
                                ...
                                onChange={(e) => setSettings(prev => ({ ...prev, accept_redirect_url: e.target.value || undefined }))}
                                placeholder="https://yoursite.com/thank-you"
                                className="w-full pl-8 pr-3 py-2 bg-white dark:bg-zinc-900 border border-gray-300 dark:border-zinc-800 rounded text-gray-900 dark:text-white text-xs placeholder-gray-400 dark:placeholder-zinc-600 focus:outline-none focus:border-blue-500 dark:focus:border-zinc-600 transition-colors"
                            />
                        </div>
                    )}
                ...
                <div>
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            const value = e.target.value
                            if (value === '_custom') {
                                setSettings(prev => ({ ...prev, reject_page_id: undefined, reject_redirect_url: '' }))
                            } else if (value === '_none') {
                                setSettings(prev => ({ ...prev, reject_page_id: undefined, reject_redirect_url: undefined }))
                            } else {
                                setSettings(prev => ({ ...prev, reject_page_id: value, reject_redirect_url: undefined }))
                            }
                        }}
                        className="w-full px-3 py-2 bg-zinc-900 border border-zinc-800 rounded text-white text-xs focus:outline-none focus:border-zinc-600 appearance-none cursor-pointer transition-colors"
                    >
                        <option value="_none">{t('funnel_components.auto_next_page')}</option>
                        {pages.map(p => (
                            <option key={p.id} value={p.id}>
                    ...
                    {!settings.reject_page_id && settings.reject_redirect_url !== undefined && settings.reject_redirect_url !== null && (
                        <div className="mt-1.5 relative">
                            ...
                            <input
                                ...
                                onChange={(e) => setSettings(prev => ({ ...prev, reject_redirect_url: e.target.value || undefined }))}
