  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-[70] h-12 bg-white dark:bg-[#080b14]/80 dark:backdrop-blur-xl border-b border-gray-200 dark:border-white/10 flex items-center justify-between pr-3 lg:pr-4 pointer-events-none transition-colors duration-200">
      ...
      <div className="flex items-center flex-1 pl-2">
        <button
          onClick={onMenuClick}
          className="lg:hidden p-1 hover:bg-gray-100 dark:hover:bg-[#1a1d2e] rounded-lg text-gray-600 dark:text-gray-400 pointer-events-auto transition-colors duration-200"
        >
          <Menu size={14} />
        </button>
      </div>
      ...
      <div className="absolute left-1/2 -translate-x-1/2 flex items-center mt-2">
        <img
          src="/logoo.png"
          alt="Logo"
          className="h-22 sm:h-22 md:h-28 w-auto object-contain"
        />
      </div>
      ...
      <div className="flex items-center gap-3 flex-1 justify-end pointer-events-auto">
        ...
        <select
          ...
          className="h-8 px-2 bg-white dark:bg-white/5 border border-gray-200 dark:border-white/10 rounded text-xs text-gray-600 dark:text-gray-300 focus:outline-none focus:ring-1 focus:ring-blue-400 cursor-pointer"
          title="Idioma"
        >
