            ...
            <div className="relative bg-white dark:bg-[#0f1420] border border-gray-200 dark:border-white/10 rounded-2xl w-full max-w-md mx-4 p-6 shadow-2xl">
                ...
                <div className="flex items-center justify-between mb-6">
                    <div className="flex items-center gap-3">
                        ...
                        <div>
                            <h2 className="text-base font-semibold text-gray-900 dark:text-white">Solicitar Antecipação</h2>
                            <p className="text-xs text-gray-500">Saldo pendente: {fmt(pendingBalance)}</p>
                        </div>
                    </div>
                ...
                <div className="mb-5 p-3 rounded-xl bg-purple-500/5 border border-purple-500/20">
                    <p className="text-xs text-purple-300 leading-relaxed">
                        Antecipe valores em <span className="font-semibold">processamento</span> para recebimento imediato, pagando uma taxa de <span className="font-semibold">{ANTICIPATION_FEE.percentage}% + {fmt(ANTICIPATION_FEE.fixed)}</span> sobre o valor antecipado.
                    </p>
                </div>

                {/* Valor */}
                <div className="mb-5">
                    <label className="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1.5">Valor a antecipar</label>
                    ...
                    {numAmount > pendingBalance && (
                        <p className="text-[11px] text-red-400 mt-1">Valor acima do saldo pendente disponível.</p>
                    )}
                </div>
                ...
                {numAmount > 0 && (
                    <div className="mb-5 p-3 rounded-xl bg-gray-50 dark:bg-white/5 border border-gray-200 dark:border-white/10 space-y-1.5">
                        ...
                        <div className="flex justify-between text-xs">
                            <span className="text-gray-500">Taxa de antecipação ({ANTICIPATION_FEE.percentage}% + {fmt(ANTICIPATION_FEE.fixed)})</span>
                ...
                <div className="flex items-start gap-2 mb-5 p-3 rounded-xl bg-amber-500/5 border border-amber-500/20">
                    ...
                    <p className="text-xs text-amber-400">
                        Antecipações estão sujeitas à análise de risco. Valores em reserva financeira (risk hold) não podem ser antecipados.
                    </p>
                </div>
