...
import { ArrowLeft, Save, Eye, Upload, X, Monitor, Smartphone, Edit3, Image as ImageIcon, Clock, Link as LinkIcon, Check, Crop, RotateCw, Menu, ShoppingCart, Tag, Key } from 'lucide-react'
...
export default function CheckoutBuilder() {
    ...
    const handleCropImage = async () => {
        // Process the cropped image from the ImageCropModal
        ...
        tempImg.onload = async () => {
            const scaleX = tempImg.naturalWidth / (imgRect.width * cropScale)
            const scaleY = tempImg.naturalHeight / (imgRect.height * cropScale)

            const visibleX = (imgRect.width * cropScale - cropRect.width) / 2 - cropPosition.x
            const visibleY = (imgRect.height * cropScale - cropRect.height) / 2 - cropPosition.y

            const sourceX = visibleX * scaleX
            const sourceY = visibleY * scaleY
            const sourceW = cropRect.width * scaleX
            const sourceH = cropRect.height * scaleY

            ctx.drawImage(tempImg, sourceX, sourceY, sourceW, sourceH, 0, 0, outputWidth, outputHeight)

            const croppedImageUrl = canvas.toDataURL('image/jpeg', 0.9)
            await handleCropComplete(croppedImageUrl)
        }
        tempImg.src = tempImageForCrop
    }

    const handleCropComplete = async (croppedImageUrl: string) => {
        const newBannerForm = {
            banner_image: croppedImageUrl,
            banner_title: bannerForm.banner_title,
            customHeight: bannerForm.customHeight || 250
        }
        ...
        // Salvar automaticamente no banco após o crop
        if (checkout) {
            try {
                setSaving(true)
                ...
                const { error } = await supabase
                    ...
                    .update({
                        banner_image: croppedImageUrl,
                        banner_title: newBannerForm.banner_title,
                        custom_height: newBannerForm.customHeight,
                        custom_fields: updatedFields
                    })
                    .eq('id', checkout.id)
                ...
                // Não mostrar alert de erro, só logar no console
                // alert('Erro ao salvar a imagem cortada')
            } finally {
                setSaving(false)
            }
        }
    ...
    return (
        <div className="min-h-screen bg-[#0f1117] flex">
                                            ...
                                            <div className="relative">
                                                ...
                                                <div className="absolute top-2 right-2 flex gap-1">
                                                    <button
                                                        ...
                                                        className="p-1.5 bg-blue-500 text-white rounded-full hover:bg-blue-600 transition-colors shadow-xl shadow-black/10"
                                                        title={t('checkout_pages.edit_image')}
                                                    >
                                                        <Crop size={14} />
                                                    </button>
                                                    <button
            ...
            <ImageCropModal
                ...
                aspectRatio="16:9"
                title={t('checkout_pages.adjust_image')}
            />
        </div>
    )
}