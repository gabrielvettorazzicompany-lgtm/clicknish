    ...
    try {
        ...
        if (request.method === 'POST' && pathSegments[0] === 'login') {
            ...
            if (!authUser) {
                ...
                if (createError) {
                    if (createError.message?.includes('already been registered') || createError.message?.includes('already exists')) {
                        ...
                        return new Response(JSON.stringify({ error: 'Erro ao criar conta: ' + (createError.message || createError.code || 'Erro desconhecido') }), {
                            status: 500,
                            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
                        })
                    }
            ...
            if (access_type === 'email-only' || access_type === 'login-simple' || !password) {
                // Sempre sincronizar a derivedPassword — garante que usuários existentes
                // também consigam login (evita mock token que quebra RLS no dashboard)
                ...
                if (loginError) {
                    // Segunda tentativa após erro inesperado
                    ...
                    if (retryError) {
                        ...
                        return new Response(JSON.stringify({ error: 'Erro ao autenticar. Tente novamente.' }), {
                            status: 401,
                            headers: { ...corsHeaders, 'Content-Type': 'application/json' }
                        })
                    } else {
