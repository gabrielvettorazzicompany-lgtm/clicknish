...
Deno.serve(async (req) => {
    ...
    // Remove /apps prefix to get relative path
    const pathAfterFunction = url.pathname.replace(/^\/apps\/?/, '')
    ...
    try {
        // GET /apps/slug/:slugOrId - Get app by slug or ID (public access)
        if (req.method === 'GET' && pathSegments.length === 2 && pathSegments[0] === 'slug') {
            const slugOrId = pathSegments[1]

            // Check if it looks like a UUID (for backwards compatibility with emails sent with ID)
        ...
        // POST /apps/verify-access - Verify user access to app (email-only)
        if (req.method === 'POST' && pathSegments.length === 1 && pathSegments[0] === 'verify-access') {
            const body = await req.json()
            const { email, appId } = body



            // Check if the user exists in app_users table
        ...
        // POST /apps/free-signup - Create free account
        if (req.method === 'POST' && pathSegments.length === 1 && pathSegments[0] === 'free-signup') {
            const body = await req.json()
            const { email, appId } = body
            ...
            // For free accounts, we'll use a different approach
            // First, let's try to create a "fake" user in auth to maintain the constraint
            // Or remove the constraint dependency
